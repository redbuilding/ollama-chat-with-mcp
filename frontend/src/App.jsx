...
const App = () => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        if (!status.service_ready) {
            ...
            // setChatHistory(prev => [...prev, {role: 'assistant', content: "MCP search service is currently unavailable."}]);
        }
      } catch (err) {
  ...
  const handleSendMessage = async (userInput) => {
    ...
    try {
      ...
      const errorMessage = err.detail || err.message || 'Failed to send message. Please check your connection or try again.';
  ...
  useEffect(() => {
    setChatHistory([
      { role: 'assistant', content: "Hello! I'm your AI assistant. Toggle the search icon to enable web search for up-to-date answers. Type #clear to reset our conversation." }
    ]);
  }, []);
